...
```bash
gc <binding> factory audit [--contract <path>] [--strict] [--require-verified]
```
...
`gc <binding> factory reconcile` is the half that checks the claims against the
...
anyone making, including you. `NONE`, `STALE` and `ERRORED` keep the findings'
own exit code by default; `--require-verified` makes them exit 4 as well, which
is the setting for a CI job that should refuse an unchecked contract.
